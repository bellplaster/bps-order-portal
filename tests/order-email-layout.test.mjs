...
test("order email uses the authenticated username, omits the lone default tab, and keeps the XLSX attachment", async (context) => {
  ...
  assert.match(requestBody.html, /https:\/\/assets\.bellplaster\.com\.au\/bell_logo_black\.png/);
  assert.doesNotMatch(requestBody.html, /bellplastersupplies\.com\.au\/cdn\/shop\/files\/bell_logo_black\.png/);
