...
test("order email uses the authenticated username, omits the lone default tab, and keeps the XLSX attachment", async (context) => {
  ...
  assert.equal(
    requestBody.subject,
    "New web portal order — Anytime Delivery to 125 Sussex St, Pascoe Vale on 1 December 2026",
  );
...
test("order email retains tab labels when multiple tabs contain products", async (context) => {
  ...
  assert.equal(
    requestBody.subject,
    "New web portal order — 1st Load Delivery to 125 Sussex St, Pascoe Vale on 1 December 2026",
  );
